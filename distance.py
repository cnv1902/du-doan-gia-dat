...
def nearest_node(graph: nx.MultiDiGraph, lat: float, lon: float) -> Any | None:
    try:
...
def add_poi_nodes(graph: nx.MultiDiGraph, pois_by_block: dict[str, list[dict[str, Any]]]) -> None:
    all_pois = [poi for pois in pois_by_block.values() for poi in pois]
    for poi in tqdm(all_pois, desc="Snapping POIs", unit="poi"):
        poi["node"] = nearest_node(graph, poi["lat"], poi["lon"])
...
def calc_distance_to_nearest_road(graph: nx.MultiDiGraph, lat: float, lon: float) -> float:
    """Calculate the perpendicular distance from a point to the nearest road edge (km)."""
    ...
    try:
        ...
        nearest_n = nearest_node(graph, lat, lon)
        if nearest_n is not None:
            n_lat = graph.nodes[nearest_n]["y"]
            n_lon = graph.nodes[nearest_n]["x"]
            return round(haversine_km(lat, lon, n_lat, n_lon) * 1000, 1)
        return 999.0
...
def process_geojson(
    ...
    for index, row in iterator:
        ...
        parcel_node = nearest_node(graph, parcel_lat, parcel_lon)
        gdf.at[index, "KC_DTG"] = calc_distance_to_nearest_road(graph, parcel_lat, parcel_lon)

        try:
            u, v, key = ox.distance.nearest_edges(graph, X=parcel_lon, Y=parcel_lat)
            edge_data = graph.get_edge_data(u, v, key)
        ...
        if pd.notna(kc_kcn) and kc_kcn is not None:
            ...
            anxh_score = 2 if kc_kcn < 1.0 else 1
        else:
            mt_score = 1
            anxh_score = 1
...
def main() -> int:
    ...
    add_poi_nodes(graph, pois_by_block)
